...
def get_rotate_crop_image(img, points):
    ...
    points = expand_bounding_box(points).astype(np.float32)
    assert points.shape == (4, 2), "shape of points must be 4*2"
    img_crop_width = int(
        max(
...
def expand_bounding_box(bbox, expand_ratio=0.2):
    ...
    new_box = np.array([[x_min_new, y_min_new], [x_max_new, y_min_new],
                        [x_max_new, y_max_new], [x_min_new, y_max_new]], dtype=np.float32)
