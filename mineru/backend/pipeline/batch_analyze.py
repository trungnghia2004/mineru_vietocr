...
class BatchAnalyze:
    ...
    def __call__(self, images_with_extra_info: list) -> list:
        ...
        for index in range(len(np_images)):
            ...
            if _lang == 'vi':
                logger.info(f"Processing page {index} with Vietnamese OCR")
        ...
        if self.table_enable:
            ...
            for _lang, rec_img_list in rec_img_lang_group.items():
                ...
                if not cropped_img_list:
                    logger.error(f"No valid cropped images for language {_lang}")
                    continue
                logger.debug(f"Processing {len(cropped_img_list)} cropped images for language {_lang}")
                ocr_res_list = ocr_engine.ocr(cropped_img_list, det=False, tqdm_enable=True, tqdm_desc=f"Table-ocr rec {_lang}")
                if not ocr_res_list or len(ocr_res_list) == 0:
                    logger.error(f"OCR model returned empty result for language {_lang}")
                    continue
        ...
        if self.enable_ocr_det_batch:
            ...
            for lang, lang_crop_list in lang_groups.items():
                if not lang_crop_list:
                    logger.error(f"No cropped images for language {lang}")
                    continue
                logger.debug(f"Processing {len(lang_crop_list)} cropped images for language {lang}")
                ...
                for group_key, group_crops in tqdm(resolution_groups.items(), desc=f"OCR-det {lang}"):
                    ...
                    target_w = ((max_w + RESOLUTION_GROUP_STRIDE - 1) // RESOLUTION_GROUP_STRIDE) * RESOLUTION_GROUP_STRIDE  # Fix: Use max_w
                    batch_images = []
        ...
        if len(img_crop_lists_by_lang) > 0:
            ...
            for lang, img_crop_list in img_crop_lists_by_lang.items():
                if not img_crop_list:
                    ...
                    continue
                if lang == 'vi':
                    logger.info(f"Using VietOCR for recognition on {len(img_crop_list)} crops")
                logger.debug(f"Processing {len(img_crop_list)} cropped images for language {lang}")
                ocr_model = atom_model_manager.get_atom_model(
                    atom_model_name=AtomicModel.OCR,
                    det_db_box_thresh=0.3,
                    lang=lang
                )
                ocr_res_list = ocr_model.ocr(img_crop_list, det=False, tqdm_enable=True, tqdm_desc=f"OCR-rec {lang}")
                if not ocr_res_list or len(ocr_res_list) == 0:
                    logger.error(f"OCR model returned empty result for language {lang}")
                    continue
                ocr_res_list = ocr_res_list[0] if len(ocr_res_list) > 0 else []  # Fix: Safe access
