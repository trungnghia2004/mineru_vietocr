...
from .tools.infer.predict_rec import TextRecognizer  # Import the new TextRecognizer
import argparse
...
cyrillic_lang = [
    'rs_cyrillic', 'bg', 'mn', 'abq', 'ady', 'kbd', 'ava',
...
class PytorchPaddleOCR(TextSystem):
    def __init__(self, *args, **kwargs):
        ...
        kwargs['limited_max_width'] = 1280
        kwargs['limited_min_width'] = 16
        
        # Lower detection threshold for Vietnamese
        if self.lang == 'vi':
            kwargs['det_db_box_thresh'] = 0.1  # Reduce threshold to increase sensitivity
            logger.info("OCR init with Vietnamese config (Paddle det + VietOCR rec)")
        ...
        # Override text_recognizer for Vietnamese
        if self.lang == 'vi':
    ...
    def ocr(self,
            img,
            det=True,
            rec=True,  # Default to rec=True to avoid det=False, rec=False
            mfd_res=None,
            tqdm_enable=False,
        ...
        with warnings.catch_warnings():
            ...
            if det and rec:
                ...
                logger.warning("Both det and rec are False, returning empty result")
                return []
